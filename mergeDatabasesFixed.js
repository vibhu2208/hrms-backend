...
async function mergeDatabases() {
  try {
    ...
    try {
      const JobPosting = recruitmentConnection.model('JobPosting', new mongoose.Schema({}, { strict: false }), 'jobpostings');
    ...
    try {
      const Department = recruitmentConnection.model('Department', new mongoose.Schema({}, { strict: false }), 'departments');
    ...
    try {
      const LeaveAccrualPolicy = recruitmentConnection.model('LeaveAccrualPolicy', new mongoose.Schema({}, { strict: false }), 'leaveaccrualpolicies');
    ...
    try {
      const LeaveBalance = recruitmentConnection.model('LeaveBalance', new mongoose.Schema({}, { strict: false }), 'leavebalances');
    ...
    try {
      ...
      for (const user of recruitmentData.users) {
        ...
        delete userObj._id; // Remove old ID
    ...
    try {
      ...
      for (const jobPosting of recruitmentData.jobPostings) {
        ...
        const existingJob = await JobPosting.findOne({ 
          title: jobPosting.title || jobPosting.jobTitle
        });
        if (existingJob) {
          console.log(`⚠️ Job posting "${jobPosting.title || jobPosting.jobTitle}" already exists, skipping...`);
          migrationResults.duplicatesSkipped++;
          continue;
        }

        const jobObj = jobPosting.toObject();
        delete jobObj._id;
        ...
        console.log(`✅ Migrated job posting: ${jobPosting.title || jobPosting.jobTitle}`);
      }
    } catch (err) {
    ...
    try {
      const Department = mainConnection.model('Department', new mongoose.Schema({}, { strict: false }), 'departments');
    ...
    try {
      const LeaveAccrualPolicy = mainConnection.model('LeaveAccrualPolicy', new mongoose.Schema({}, { strict: false }), 'leaveaccrualpolicies');
    ...
    try {
      const LeaveBalance = mainConnection.model('LeaveBalance', new mongoose.Schema({}, { strict: false }), 'leavebalances');
    ...
    try {
      ...
      const result = await CompanyRegistry.deleteMany({ tenantDatabaseName: recruitmentTenantDb });
      console.log(`✅ Removed ${result.deletedCount} recruitment tenant references from global registry`);
    } catch (err) {
