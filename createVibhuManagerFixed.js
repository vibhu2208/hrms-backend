...
async function createVibhuManager() {
  try {
    // Connect to main database first to get company info
    ...
    companies.forEach(company => {
      console.log(`- ${company.name || 'No Name'} (${company._id})`);
    });

    // Use the first available company
    const company = companies[0];
    console.log(`✅ Using company: ${company.name || 'No Name'} (${company._id})`);
    console.log(`🏢 Tenant DB: tenant_${company._id}`);

    // Connect to tenant database
    const tenantDbName = `tenant_${company._id}`;
