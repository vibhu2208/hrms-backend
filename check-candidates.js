...
async function checkCandidates() {
  try {
    ...
    if (totalCandidates > 0) {
      const sampleCandidates = await Candidate.find()
        .limit(5)
        .select('firstName lastName email skills experience currentLocation status isActive preferredLocation candidateCode');
      ...
      if (javaCandidates.length > 0) {
        ...
        console.log('❌ No Java developers found');
      }
    }
